...
- **Flow**: Articles → Scraper → Chunks → Embeddings → Qdrant → MCP Tools
...
- **Strategy Pipeline**: Video → NPRD → Analyst → Trading Bot Code
...
```
QUANTMINDX/
├── scripts/              # Data collection & indexing
│   ├── firecrawl_scraper.py    # Web scraper
│   ├── category_crawler.py     # Automated link discovery
│   ├── index_to_qdrant.py      # KB indexer
│   ├── kb.sh                   # KB manager CLI
│   └── filter_analyst_kb.py    # KB filtering
│
├── tools/
│   ├── nprd/                    # Video indexer
│   │   ├── gemini_client.py     # Gemini 2.0 Flash
│   │   ├── zai_client.py        # ZhipuAI GLM-4.5V
│   │   ├── openrouter_client.py # OpenRouter integration
│   │   ├── video_splitter.py    # Chunking logic
│   │   └── markdown_generator.py # Output generation
│   ├── nprd_cli.py              # NPRD CLI entry point
│   └── analyst_agent/           # Strategy extraction agent
...
├── mcp-servers/
│   └── quantmindx-kb/
│       ├── server.py            # Qdrant MCP server
│       ├── server_chroma.py     # ChromaDB variant
│       └── start.sh             # Server launcher
│
├── mcp-metatrader5-server/      # MT5 MCP integration
│   ├── src/mcp_mt5/
│   │   └── main.py              # FastMCP server
│   └── docs/                    # MT5 API documentation
│
├── data/
│   ├── inputs/                  # Article URLs
│   ├── scraped_articles/        # Scraped content (markdown)

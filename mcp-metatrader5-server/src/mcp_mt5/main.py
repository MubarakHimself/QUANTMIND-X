...
mcp = FastMCP(
    ...
    EA Management Tools (v2.0):
    - list_installed_eas: List all EAs in the Experts folder
    - get_ea_info: Get details about a specific EA
    - get_ea_status: Check if an EA is active by magic number
    - get_ea_performance: Get trading performance metrics for an EA
    - create_ea_template: Create a deployment template for an EA
    - stop_ea_by_magic: Emergency stop - close all positions/orders for an EA
    - get_daily_pnl: Get daily profit/loss summary
    
    Account Management Tools:
    - unlock_vault: Unlock encrypted credential vault
    - lock_vault: Lock vault and clear sensitive data
    - list_accounts: List configured MT5 accounts
    - add_account: Add new MT5 account credentials
    - remove_account: Remove an account from vault
    - switch_account: Switch to different MT5 account
    - get_connection_status: Get current connection state
    - disconnect_account: Disconnect from MT5
    
    Alert Tools:
    - configure_email_alerts: Set up email notifications
    - test_email_alert: Verify email configuration
    - send_trading_alert: Send alert notification
    - get_alert_history: Get recent alerts
    - get_alert_config: View alert settings
    
    Trade Journal Tools:
    - sync_journal_with_mt5: Import trades from MT5 to journal DB
    - annotate_trade: Add notes/tags/ratings to a trade
    - get_journal_stats: Get win rate, profit factor, etc.
    - search_journal: Search trades by symbol, setup, status
    - export_journal: Export trades to CSV or JSON
    """,
)

# Register EA management tools
from .ea_tools import register_ea_tools
register_ea_tools(mcp)
...
from .journal_tools import register_journal_tools
register_journal_tools(mcp)

# Register Paper Trading tools
from .paper_trading.tools import register_paper_trading_tools
register_paper_trading_tools(mcp)

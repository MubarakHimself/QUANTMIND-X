<script lang="ts">
  ...
  // Wrapper to handle context menu event from ChatItem
  function handleContextMenuEvent(event: CustomEvent<MouseEvent>, chat: Chat) {
    openContextMenu(event.detail, chat);
  }
    ...
    {#if filteredPinned.length > 0}
      ...
      {#each filteredPinned as chat (chat.id)}
        <ChatItem
          ...
          on:contextMenu={(e) => handleContextMenuEvent(e, chat)}
        />
      {/each}
    {/if}

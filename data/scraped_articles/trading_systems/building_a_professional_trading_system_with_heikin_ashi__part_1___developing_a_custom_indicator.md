...
relevance_score: 12
scraped_at: 2026-01-22T17:13:40.311672
---

[What's wrong with regular VPS?Here are the 8 most common problems that algorithmic traders may encounterRead![](https://www.mql5.com/ff/sh/hzatb686qjqxwtr4z2/01.png)](https://www.mql5.com/ff/go?link=https://www.mql5.com/en/forum/450486&a=drhremihlwuaqyvgpzfddbtmgciejpba&s=c37d25bcceb93ed153b814e6ba4d4839461a9b2d68dd82b95b142be06d310f3f&uid=&ref=https://www.mql5.com/en/articles/19260&id=bfogggabsofabcpxuzmgaibarmaxasdrj&fz_uniq=5048979982329422482)
...
1. Open is the price at the start of candlestick formation.
2. High is the highest price reached during candlestick formation.
3. Low is the lowest price reached.
4. Close is the final price at the end of candlestick formation.
...
1. The Heikin Ashi Open, which is a smoothed version of the candlestick open.
2. The Heikin Ashi High, which is the highest value from a set that includes both real and smoothed data.
3. The Heikin Ashi Low, which is the lowest value from the same kind of set.
4. The Heikin Ashi Close, which is the average price of the current real candle.
...
1. Heikin Ashi Open. This is an array that stores the Heikin Ashi open price.
2. Heikin Ashi High. This is an array that stores the Heikin Ashi high price.
3. Heikin Ashi Low. This is an array that stores the Heikin Ashi low price.
4. Heikin Ashi Close. This is an array that stores the Heikin Ashi close price.
5. Color Buffer. This is an array that stores the color index for each Heikin Ashi candle depending on whether it is bullish, bearish, or neutral.
...
- Rates\_total. This refers to the total bars available.
- Prev\_calculated. This is how many bars were processed during the last call of the 'OnCalculate' function (for optimization).
- Time, open, high, low, and close variables, which are basically our historical price time series.
- Volume, tick\_volume, and spread, which are additional trading data.
...
To define how an indicator should be displayed, we use either #property indicator\_chart\_window for chart overlays or #property indicator\_sub\_window for subwindow-based indicators. Since we are building an indicator that overlays price, we want it to appear right on the main chart.
...
Before you can use them in your source code, you need to specify how many buffers your indicator will use. This is done using the directive #property indicator\_buffers N. Here N represents the total number of buffers that your indicator needs. In our case, we are going to use 5 buffers, so we'll declare it like this: #property indicator\_buffers 5.
...
The number of graphic plots an indicator will display is set using the directive #property indicator\_plots P, where P is the number of graphic plots to be displayed by the indicator. In our case, we are only working with a single graphic plot.
...
1. When the indicator is first attached to the chart, this is when prev\_calculated == 0. We use this block to perform any initialization logic, like filling in historical values or setting up starting conditions.
2. When a new candle opens, this is when prev\_calculated != rates\_total and prev\_calculated != 0. Here, you can perform logic that only needs to run once per new bar. It is useful for reducing repetitive calculations.
3. When a new price tick arrives on the current bar, this is when prev\_calculated == rates\_total. In this block, we update real-time values for the current candle using the latest price data.
...
Next, we are going to define a color palette for our Heikin Ashi candles using the #property indicator\_color1 directive. This directive allows us to specify the default colors for our graphical plot we defined earlier. For our indicators, we'll be using three colors:

1. clrDarkGreen for bullish Heikin Ashi candles.
2. clrDarkRed for bearish Heikin Ashi candles.
3. clrYellow for neutral candles.
...
- When prev\_calculated == 0, meaning the indicator is loading for the first time and we need to process all historical candles.
- When prev\_calculated != rates\_total and prev\_calculated != 0, meaning new bars have been added or the chart has been refreshed, so we must recalculate from where we left off.
...
- When prev\_calculated == 0, color all historical candles during the initial load.
- When prev\_calculated != rates\_total and prev\_calculated != 0, refresh the colors for the updated data set.
...
Finally, we'll call the functions GetCurrentHeikinAshiValue() and GetCurrentHeikinAshiColor() inside the block if(prev\_calculated == rates\_total){}. This block executes when the indicator is updating in real time. That is, when a new tick arrives but no new candle has formed yet.
...
Now, let us make sure that the Heikin Ashi data shows up in the data window when you hover over the chart with your mouse. We do this using PlotIndexSetInteger() with PLOT\_SHOW\_DATA set to true. This line should also go right after the last one inside the OnInit() function. If it doesn't work, it will print an error message.
...
Next, we are going to set up some general settings for the indicator. The first setting will define how many decimal places (digits) the indicator values should have, using IndicatorSetInteger() with INDICATOR\_DIGITS. This helps ensure that the indicator's values match the symbol's precision. If something goes wrong, we print an error message to help with debugging.
...
[Download ZIP](https://www.mql5.com/en/articles/download/19260.zip "Download all attachments in the single ZIP archive")

[heikinAshiIndicator.mq5](https://www.mql5.com/en/articles/download/19260/heikinAshiIndicator.mq5 "Download heikinAshiIndicator.mq5")(8.66 KB)

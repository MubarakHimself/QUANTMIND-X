...
class FeeMonitor:
    ...
    def should_halt_trading(self) -> Tuple[bool, str]:
        """Checks if kill switch should activate"""
        ...
        if report.fee_burn_pct > self.MAX_DAILY_FEE_PCT * 100:
            return True, f"Daily fee burn {report.fee_burn_pct:.1f}% exceeds {self.MAX_DAILY_FEE_PCT*100}% limit"
        if report.fee_per_trade > self.MAX_FEE_PER_TRADE:
            return True, f"Fee per trade ${report.fee_per_trade:.2f} > ${self.MAX_FEE_PER_TRADE}"

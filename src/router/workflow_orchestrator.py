...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        self.internal_api_base_url = get_internal_api_base_url()

        logger.info(f"WorkflowOrchestrator initialized with work_dir={work_dir}")

    def _approval_gate_url(self, suffix: str = "") -> str:
        """Build an approval-gate URL from the configured internal API base."""
        return f"{self.internal_api_base_url}/api/approval-gates{suffix}"
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        self._approval_gate_url(f"/{gate_id}"),
                        timeout=aiohttp.ClientTimeout(total=10)
                    ) as response:
    ...
    async def _run_analyst(
        ...
        try:
            ...
            raise RuntimeError(
                "Analyst graph is unavailable; cannot continue production workflow orchestration"
            ) from e
        except Exception as e:
    ...
    async def _run_quantcode(
        ...
        try:
            ...
            raise RuntimeError(
                "QuantCode graph is unavailable; refusing to generate placeholder EA code"
            ) from e
        except Exception as e:
    ...
    async def _compile_code(
        ...
        try:
            ...
            raise RuntimeError(
                "MCP compiler tools are unavailable; refusing to report fake compilation success"
            ) from e
        except Exception as e:
    ...
    async def _run_backtest(
        ...
        try:
            ...
            raise RuntimeError(
                "MCP backtest tools are unavailable; refusing to fabricate backtest metrics"
            ) from e
        except Exception as e:
